...
const Topproduct = () => {
  ...
  const fetchTopProducts = async () => {
    try {
      const response = await axios.get('/user/product/getpopularproduct');
      setTopProducts(response.data.popularProducts || []); // Ensure we set to an empty array if undefined
    } catch (error) {
  ...
  const filteredProducts = topProducts.filter((product) => {
    const productId = product.productId || {}; // Ensure productId exists
    const title = productId.title ? productId.title.toLowerCase() : ''; // Ensure title exists
    const description = productId.description
      ? productId.description.toLowerCase()
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <button
        className="mb-4 bg-blue-600 text-white py-2 px-4 rounded-lg shadow hover:bg-blue-700 transition duration-300 ease-in-out"
        onClick={() => navigate('/dashboard')} // Navigate to /dashboard on click
      >
        Back to Dashboard
      </button>
      <h1 className="text-3xl font-bold mb-8 text-center">Most Popular Product</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        {filteredProducts.length > 0 ? ( // Check if filteredProducts is not empty
          filteredProducts.map((product) => {
            ...
            if (!productId) return null; // Skip rendering if productId is null
            ...
            return (
              ...
              >
                <img
                  src={productId.images[0]} // Assuming the first image is the main one
                  alt={productId.title}
                  className="h-48 w-full object-cover"
                />
                <div className="p-4">
                  <h2 className="text-lg font-bold">{productId.title}</h2>
                  <p className="text-gray-600 mt-2">{productId.description}</p>
                  <div className="mt-4 flex items-center">
                    <span className="text-yellow-500">★</span>
                    <span className="ml-2 text-gray-700 font-bold">
                      {productId.totalrating?.toFixed(1) || 'N/A'}
                    </span>
                  </div>
               ...
               <Link to={`/dashboard/viewproduct/${productId._id}`}>
